...
class  ticketC
{
    ...
    {
    $sql = "INSERT INTO ticket VALUES (NULL, :date,:prix,:date_limite,:quantite,:heure,:lieu,:concert,:image,:quantite_ticket)";
    $db = config::getConnexion();
    try 
    {
        ...
        $query->execute([
            ...
            'quantite_ticket' => $ticket->getquantite_ticket()
        ...
        try {
            ...
            (
                'UPDATE ticket SET 
                    ...
                    concert = :concert,
                    image=:image,
                    quantite_ticket=:quantite_ticket
...
{
    // Check if $id_concert is not provided
    if ($id_concert === null) {
        die('Error: No id_concert parameter provided.');
    }
    ...
    try {
        ...
        if ($ticket === false) {
            die('Error fetching ticket data.');
        }
